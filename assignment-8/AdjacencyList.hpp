...
public:
	AdjacencyList(int v) : l(v, list<GraphEdge> ()) {
	}
	...
	virtual bool edgeExits(int i, int j) const override {
		if (is_valid_index(i, j)) {
			return l.at(i).has(GraphEdge(j, 0));
		} else {
			return false;
		}
	}
	...
	virtual void add(int i, int j, int weight) override {
		...
		if (!l[i].has(GraphEdge(j, 0))) {
			l[i].append(GraphEdge(j, weight));
		} else {
			(*l[i].search(GraphEdge(j, 0))).weight = weight;
		}
	}
	/*
	 * Function: remove
	...
	const LinearList<list<GraphEdge> >& peak() const {
		return l;
	}
};
